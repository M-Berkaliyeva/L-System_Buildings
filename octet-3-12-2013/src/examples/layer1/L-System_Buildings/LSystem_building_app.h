...
class LSystem_building_app : public app {
	...
	void draw_world(int x, int y, int w, int h) {
		...
		int i = 0;
		font->draw_string(1.f, 1.f, 1.f, 0, 720, w, h, l[BUILDING_INDEX_TO_MODIFY].get_string().c_str());
		font->draw_string(1.f, 1.f, 1.f, w1 + 225, h1 - i, w, h, "hotkey");
		i += DELTA;
		font->draw_string(1.f, 1.f, 1.f, w1, h1 - i, w, h, "iteration: %d", l[BUILDING_INDEX_TO_MODIFY].get_iteration());
		...
		i += DELTA;
		font->draw_string(1.f, 1.f, 1.f, w1, h1 - i, w, h, "floor_count: %d", l[BUILDING_INDEX_TO_MODIFY].get_floor_count());
