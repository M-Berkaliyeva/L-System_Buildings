...
#include "L-System_Buildings/LSystem_building_app.h"
...
namespace octet {
  static app *app_factory(const char *name, int argc, char **argv) {
    ...
    else if (!strcmp(name, "LSystem_building")) return new LSystem_building_app(argc, argv); //  physics sample: continue here to learn how to annimate using physics
    else return 0;
  }
